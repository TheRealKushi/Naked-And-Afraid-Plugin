 ...
 * fatal damage and force death if a totem is present. Compatible with Minecraft 1.17–1.18.2.
 * <p>
 * Contributors:
...
public class TotemDisablerListener implements Listener {
    ...
    public TotemDisablerListener(NakedAndAfraid nakedAndAfraid) {
        ...
        if (!plugin.getConfig().getBoolean("disable-totems", true)) {
            nakedAndAfraid.debugLog("[TotemDisablerListener] Totem disabling is turned off in config.");
            return;
        }

        if (registerTotemDeathEvent()) {
            totemEventRegistered = true;
            nakedAndAfraid.debugLog("[TotemDisablerListener] Successfully registered PlayerTotemDeathEvent (Paper 1.17+ detected).");
        } else {
            nakedAndAfraid.debugLog("[TotemDisablerListener] Using EntityDamageEvent fallback for Spigot or pre-1.17.");
        }
    ...
    private boolean registerTotemDeathEvent() {
        try {
            ...
            if (!Event.class.isAssignableFrom(totemEventClass)) {
                nakedAndAfraid.debugLog("[TotemDisablerListener] PlayerTotemDeathEvent does not extend org.bukkit.event.Event.");
                return false;
            }
            ...
            EventExecutor executor = (listener, event) -> {
                if (totemEventClass.isInstance(event)) {
                    try {
                        ...
                        nakedAndAfraid.debugLog("[TotemDisablerListener] Detected totem activation for player " +
                                player.getName() + ", cancelling PlayerTotemDeathEvent.");
                        setCancelledMethod.invoke(event, true);
                    } catch (Exception e) {
                        nakedAndAfraid.debugLog("[TotemDisablerListener] Failed to cancel PlayerTotemDeathEvent: " + e.getMessage());
                    }
                }
            };
            ...
            nakedAndAfraid.debugLog("[TotemDisablerListener] PlayerTotemDeathEvent not available: " + e.getMessage());
            return false;
        }
    }
    ...
    public void onEntityDamage(EntityDamageEvent event) {
        ...
        if (totemEventRegistered) {
            nakedAndAfraid.debugLog("[TotemDisablerListener] Skipping EntityDamageEvent due to PlayerTotemDeathEvent usage.");
            return;
        }
        ...
        if (event.getEntity() instanceof Player player) {
            ...
            nakedAndAfraid.debugLog("[TotemDisablerListener] Checking damage for player " + player.getName() +
            ...
            //     nakedAndAfraid.debugLog("[TotemDisablerListener] Allowing totem in world " + player.getWorld().getName());
            //     return;
            // }
            ...
            if (finalDamage >= health) {
                ...
                if ((mainHand != null && mainHand.getType() == Material.getMaterial("TOTEM")) ||
                        (offHand != null && offHand.getType() == Material.getMaterial("TOTEM"))) {
                    nakedAndAfraid.debugLog("[TotemDisablerListener] Totem detected for player " + player.getName() +
                    ...
                    //     nakedAndAfraid.debugLog("[TotemDisablerListener] Consumed totem from main hand for " + player.getName());
                    // } else if (offHand != null && offHand.getType() == Material.TOTEM_OF_UNDYING) {
                    //     player.getInventory().setItemInOffHand(null);
                    //     nakedAndAfraid.debugLog("[TotemDisablerListener] Consumed totem from offhand for " + player.getName());
                    // }
    ...
    private boolean isAdventureSupported() {
        try {
            ...
            nakedAndAfraid.debugLog("[TotemDisablerListener] Failed to parse Bukkit version: " + e.getMessage());
            return false;
        }
    }
}