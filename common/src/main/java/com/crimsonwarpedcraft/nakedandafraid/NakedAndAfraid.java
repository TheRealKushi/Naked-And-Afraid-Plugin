...
public class NakedAndAfraid extends JavaPlugin {
  ...
  public void onEnable() {
    ...
    getServer().getPluginManager().registerEvents(new VersionNotifyListener(this), this);
  ...
  /**
   * Sends a message to the command sender, using Adventure API if supported, otherwise legacy color codes.
   *
   * @param sender  The command sender to send the message to.
   * @param message The message to send (using § color codes or plain text).
   */
  private void sendMessage(CommandSender sender, String message) {
    if (isAdventureSupported()) {
      Component component = Component.text(message.replace("§a", "").replace("§c", "").replace("§e", "").replace("§6", ""))
              .color(message.contains("§a") ? NamedTextColor.GREEN :
                      message.contains("§c") ? NamedTextColor.RED :
                              message.contains("§e") ? NamedTextColor.YELLOW :
                                      message.contains("§6") ? NamedTextColor.GOLD : NamedTextColor.WHITE);
  ...
  public void reloadListeners() {
    ...
    if (getConfig().getBoolean("disable-chat", true)) {
      ...
      getLogger().info("Naked And Afraid - Chat Restriction Disabled.");
    ...
    if (getConfig().getBoolean("disable-join-quit-messages", true)) {
      ...
      getLogger().info("Naked And Afraid - Message Disabling Disabled.");
      debugLog("[NakedAndAfraid] JoinQuitMessageSuppressor not enabled");
    }

    // Totem disabling
    if (getConfig().getBoolean("disable-totems", true)) {
      totemDisablerListener = new TotemDisablerListener(this);
      getServer().getPluginManager().registerEvents(totemDisablerListener, this);
      getLogger().info("Naked And Afraid - Totem Disabling Enabled.");
      debugLog("[NakedAndAfraid] Enabled TotemDisablerListener");
    } else {
      getLogger().info("Naked And Afraid - Totem Disabling Disabled.");
  ...
  private void setWorldList() {
    ...
    try {
      ...
      var existingWorlds = new LinkedHashMap<String, Boolean>();
      var enabledWorldsLineIndices = new ArrayList<Integer>();
          ...
          if (inEnabledWorlds) {
            if (!trimmed.isEmpty() && !trimmed.startsWith("#") && !line.startsWith(" ") && trimmed.contains(": ")) {
              ...
              if (parts.length > 1) {
                ...
                if (!worldName.matches(".*-.*") && !worldName.matches("\\d+") && !worldName.isEmpty()) {
      ...
      if (needsRewrite) {
        ...
        // Remove old world lines
        for (var idx : enabledWorldsLineIndices) {
          if (idx < lines.size()) {
            lines.set(idx, "");
        ...
        // Insert all worlds after the key line
        String finalEnabledWorldsLine = enabledWorldsLine;
        var worldLines = existingWorlds.entrySet().stream()
                .map(entry -> {
                  String indent = "  ";
  ...
  public boolean onCommand(@Nonnull CommandSender sender, @Nonnull Command command, @Nonnull String label, @Nonnull String[] args) {
    ...
    if (args.length == 0 || args[0].equalsIgnoreCase("help")) {
      ...
      if (args.length >= 2) {
        try {
          ...
          sendMessage(sender, "§cInvalid help page number. Showing page 1.");
    ...
    switch (args[0].toLowerCase()) {
      case "reloadconfig":
        ...
        sendMessage(sender, "§aNaked and Afraid config reloaded.");
      ...
      case "spawn":
        ...
        boolean spawnResult = spawnManager.handleCommand(sender, args);
        debugLog("[NakedAndAfraid] Spawn command result for " + sender.getName() + ": " + spawnResult);
        return spawnResult;
      ...
      case "team":
        ...
        boolean teamResult = teamCommands.handleTeamCommand(sender, args);
        debugLog("[NakedAndAfraid] Team command result for " + sender.getName() + ": " + teamResult);
        return teamResult;
      ...
      case "user":
        ...
        boolean userResult = teamCommands.handleUserCommand(sender, args);
        debugLog("[NakedAndAfraid] User command result for " + sender.getName() + ": " + userResult);
        return userResult;

      default:
        sendMessage(sender, "§cUnknown subcommand. Use /nf help for commands.");
  ...
  public List<String> onTabComplete(@Nonnull CommandSender sender, @Nonnull Command command, @Nonnull String alias, @Nonnull String[] args) {
    ...
    if (args.length == 1) {
      var completions = List.of("help", "reloadconfig", "spawn", "team", "user");
    ...
    switch (args[0].toLowerCase()) {
      case "spawn":
        if (args.length == 2) {
          var completions = List.of("create", "rename", "remove", "list", "tp", "tpall");
        ...
        if (args.length == 3) {
          ...
          if (args[1].equalsIgnoreCase("rename") || args[1].equalsIgnoreCase("remove") || args[1].equalsIgnoreCase("tp")) {
            var completions = spawnManager.getSpawns().keySet().stream().toList();
        ...
        if (args.length == 4) {
          if (args[1].equalsIgnoreCase("create") || args[1].equalsIgnoreCase("tp")) {
            var completions = Bukkit.getOnlinePlayers().stream().map(Player::getName).toList();
      ...
      case "team":
        if (args.length == 2) {
          var completions = List.of("create", "remove", "list", "block", "setblock", "meta");
        ...
        if (args.length == 3) {
          if (args[1].equalsIgnoreCase("remove") || args[1].equalsIgnoreCase("setblock") || args[1].equalsIgnoreCase("block")) {
            var completions = teamsManager.getTeams().stream().map(TeamsManager.Team::getName).toList();
        ...
        if (args.length == 4) {
          if (args[1].equalsIgnoreCase("create")) {
            var completions = ValidTeamColors().stream()
          ...
          if (args[1].equalsIgnoreCase("setblock") && sender instanceof Player player) {
            var completions = List.of(String.valueOf(player.getLocation().getBlockX()));
          ...
          if (args[1].equalsIgnoreCase("meta") && args[2].equalsIgnoreCase("color")) {
            var completions = List.of("get", "set");
        ...
        if (args.length == 5) {
          if (args[1].equalsIgnoreCase("setblock") && sender instanceof Player player) {
            var completions = List.of(String.valueOf(player.getLocation().getBlockY()));
        ...
        if (args.length == 6 && args[1].equalsIgnoreCase("setblock") && sender instanceof Player player) {
          var completions = List.of(String.valueOf(player.getLocation().getBlockZ()));
      ...
      case "user":
        ...
        if (args.length == 3 && args[2].equalsIgnoreCase("team")) {
          var completions = List.of("add", "remove", "list");
   ...
   */
  private void sendHelpMessage(@Nonnull CommandSender sender, int page) {
    debugLog("[NakedAndAfraid] Preparing help message for " + sender.getName() + ", page " + page);
    var helpLines = List.of(
